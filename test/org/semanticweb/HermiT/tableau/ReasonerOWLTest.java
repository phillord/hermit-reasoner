...
public class ReasonerOWLTest extends AbstractOWLOntologyTest {

    public ReasonerOWLTest(String name) {
        super(name);
    }
	public void testReflexivity() throws Exception {
	    ...
	    createOntologyWithAxioms(axioms);
    ...
    public void testExistsSelf1() throws Exception {
    	loadResource("res/self-1-input.owl");
    	assertABoxSatisfiable(false);
    }
    public void testExistsSelf2() throws Exception {
    	loadResource("res/self-2-input.owl");
    	assertABoxSatisfiable(true);
    }
    public void testAsymmetry() throws Exception {
    	loadResource("res/asymmetric-1-input.xml");
    	assertABoxSatisfiable(false);
    }
    public void testSatisfiability1() throws Exception {
        String axioms = "ClassAssertion(a C) ClassAssertion(a ObjectComplementOf(C))";
        createOntologyWithAxioms(axioms);
        assertABoxSatisfiable(false);
    }
    ...
    public void testSatisfiability3() throws Exception {
        String axioms = "SubClassOf(Person ObjectSomeValuesFrom(hasParent Person)) " +
        		"SubClassOf(ObjectSomeValuesFrom(hasParent ObjectSomeValuesFrom(hasParent Person)) Grandchild)" + 
        		"ClassAssertion(peter Person)" +
        		"ClassAssertion(peter ObjectComplementOf(Grandchild))";
        createOntologyWithAxioms(axioms);
        assertABoxSatisfiable(false);
    }
    ...
    public void testHeinsohnTBox1() throws Exception {
		...
		"SubClassOf(f d)" +
		"SubClassOf(c1 d1)" +
		"DisjointClasses(c1 d1)" +
		"EquivalentClasses(complex1 ObjectIntersectionOf(c d))" +
		"EquivalentClasses(complex2 ObjectIntersectionOf(ObjectAllValuesFrom(r ObjectIntersectionOf(c d)) ObjectSomeValuesFrom(r owl:Thing)))" +
		"EquivalentClasses(complex3 ObjectIntersectionOf(e3 f))";
        createOntologyWithAxioms(axioms);
    ...
    public void testHeinsohnTBox2() throws Exception {
    	...
    	String axioms = "DisjointClasses(c d)" +
		"EquivalentClasses(complex1 ObjectIntersectionOf(ObjectMinCardinality(2 r) ObjectMaxCardinality(1 r)))" +
		"EquivalentClasses(complex2 ObjectIntersectionOf(ObjectMaxCardinality(1 r) ObjectSomeValuesFrom(r c) ObjectSomeValuesFrom(r d)))";
        createOntologyWithAxioms(axioms);
        assertSatisfiable("complex1",false);
        assertSatisfiable("complex2",false);
    }
    public void testHeinsohnTBox3c() throws Exception {
    	...
    	"SubObjectPropertyOf(t1 tc)" +
    	"SubObjectPropertyOf(t1 td)" +
    	"SubClassOf(owl:Thing ObjectAllValuesFrom(tc c))" +
    	"SubClassOf(owl:Thing ObjectAllValuesFrom(td d))" +
    	"SubObjectPropertyOf(tc r)" +
    	"SubObjectPropertyOf(td s)" +
		"EquivalentClasses(complex1 ObjectIntersectionOf(ObjectAllValuesFrom(t1 a) ObjectMinCardinality(3 t1) ObjectMaxCardinality(1 r) ObjectMaxCardinality(1 s)))";
    	createOntologyWithAxioms(axioms);
    	assertSatisfiable("complex1", false);
    }
