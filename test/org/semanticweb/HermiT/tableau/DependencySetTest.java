...
public class DependencySetTest extends TestCase {
    ...
    public void testDependencySet1() {
        ...
        PermanentDependencySet set2 = m_factory.addBranchingPoint(
                m_factory.emptySet(), 0);
        assertTrue(set != set2);
        assertSame(set, m_factory.addBranchingPoint(set2, 32));
        set = m_factory.unionWith(set, m_factory.addBranchingPoint(
                m_factory.addBranchingPoint(set2, 15), 17));
        assertDSEquals(set, 0, 15, 17, 32);
        assertSame(set, m_factory.unionWith(set, m_factory.emptySet()));
        assertSame(set, m_factory.addBranchingPoint(m_factory.unionWith(set,
                m_factory.emptySet()), 17));
    ...
    public void testDependencySet3() {
        ...
        union.getConstituents()[0] = set1;
        union.getConstituents()[1] = set2;
        union.getConstituents()[2] = set3;
        PermanentDependencySet set4 = m_factory.getPermanent(union);
        assertDSEquals(set4, 1, 2, 3, 10, 14, 17, 18);
    }

    protected static void assertDSEquals(PermanentDependencySet dependencySet,
            int... expectedSortedMembers) {
