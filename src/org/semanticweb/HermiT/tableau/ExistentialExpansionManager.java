...
public final class ExistentialExpansionManager implements Serializable {
    ...
    private boolean getFunctionalExpansionNode(Role abstractRole,Node forNode,Object[] result) {
        Role[] relevantRoles=m_functionalRoles.get(abstractRole);
    ...
    private Map<Role,Role[]> buildFunctionalRoles() {
        ...
        for (Role abstractRole : roleHierarchy.getAllObjects()) {
            Set<Role> relevantRoles=new HashSet<Role>();
            Set<Role> allSuperroles=roleHierarchy.getAllSuperobjects(abstractRole);
            ...
            if (!relevantRoles.isEmpty()) {
                ...
                result.put(abstractRole,relevantRolesArray);
            }
        }
        return result;
    }
