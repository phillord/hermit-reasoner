...
public class Classifier<T> implements Serializable {
    ...
    public Map<T,HierarchyPosition<T>> buildHierarchy(T top,T bottom,Collection<T> values) {
        Map<T,HierarchyPosition<T>> out=new HashMap<T,HierarchyPosition<T>>();
        NaiveHierarchyPosition<T> topPos=new NaiveHierarchyPosition<T>(top);
        out.put(top,topPos);
        if (func.doesSubsume(bottom,top)) {
            ...
            NaiveHierarchyPosition<T> botPos=new NaiveHierarchyPosition<T>(bottom);
            ...
            for (T t : values) {
                ...
                if (!pos.getEquivalents().contains(t)) {
                    ...
                    for (HierarchyPosition<T> parent : pos.getParentPositions()) {
                        NaiveHierarchyPosition<T> p=(NaiveHierarchyPosition<T>)parent;
                        p.children.add(pos);
                        p.children.removeAll(pos.getChildPositions());
                    }
                    for (HierarchyPosition<T> child : pos.getChildPositions()) {
                        NaiveHierarchyPosition<T> c=(NaiveHierarchyPosition<T>)child;
                        c.parents.add(pos);
    ...
    public HierarchyPosition<T> findPosition(T value,HierarchyPosition<T> hierTop,HierarchyPosition<T> hierBottom) {
    ...
    protected Set<HierarchyPosition<T>> findDirectSupers(final T value,HierarchyPosition<T> top) {
        Util<HierarchyPosition<T>> util=new Util<HierarchyPosition<T>>() {
            ...
            public boolean trueOf(HierarchyPosition<T> u) {
                return func.doesSubsume(u.getEquivalents().iterator().next(),value);
            }
        };
    ...
    protected Set<HierarchyPosition<T>> findDirectSubs(final T value,HierarchyPosition<T> hierBottom,Set<HierarchyPosition<T>> supers) {
        if (supers.size()==1 && func.doesSubsume(value,supers.iterator().next().getEquivalents().iterator().next())) {
            ...
            Set<HierarchyPosition<T>> marked=new HashSet<HierarchyPosition<T>>(i.next().getDescendantPositions());
            while (i.hasNext()) {
                Set<HierarchyPosition<T>> freshlyMarked=new HashSet<HierarchyPosition<T>>();
                ...
                q.add(i.next());
                while (!q.isEmpty()) {
                    for (HierarchyPosition<T> pos : q.remove().getChildPositions()) {
            ...
            Util<HierarchyPosition<T>> util=new Util<HierarchyPosition<T>>() {
                ...
                public boolean trueOf(HierarchyPosition<T> u) {
                    return func.doesSubsume(value,u.getEquivalents().iterator().next());
                }
            };
            if (bots.isEmpty()) {
