...
public class NaiveHierarchyPosition<T> implements Serializable,HierarchyPosition<T> {
    ...
    public NaiveHierarchyPosition(T label,Set<HierarchyPosition<T>> parents,Set<HierarchyPosition<T>> children) {
    ...
    protected Set<HierarchyPosition<T>> topSearch(T val,Ordering<T> cmp) {
    ...
    protected Set<HierarchyPosition<T>> botSearch(T val,Ordering<T> cmp) {
        Set<HierarchyPosition<T>> out=new HashSet<HierarchyPosition<T>>();
        Set<HierarchyPosition<T>> visited=new HashSet<HierarchyPosition<T>>();
        Queue<HierarchyPosition<T>> q=new LinkedList<HierarchyPosition<T>>();
        q.add(this);
    ...
    public static <T> Map<T,HierarchyPosition<T>> buildHierarchy(T topVal,T botVal,Collection<T> values,Ordering<T> cmp) {
            ...
            for (T t : values) {
                ...
                if (parents.equals(children)) {
                    ...
                    for (HierarchyPosition<T> par : parents) {
                        NaiveHierarchyPosition<T> p=(NaiveHierarchyPosition<T>)par;
                        p.children.add(pos);
    ...
    public static interface Ordering<T> {
