...
public class OwlClausification {
    ...
    public DLOntology clausify(boolean prepareForNIRule,
        ...
        Clausifier clausifier=new Clausifier(positiveFacts,shouldUseNIRule,factory);
        for (OWLDescription[] inclusion : conceptInclusions) {
            for (OWLDescription description : inclusion) description.accept(clausifier);
        ...
        for (OWLIndividualAxiom fact : facts) fact.accept(factClausifier);
        for (DescriptionGraph descriptionGraph : descriptionGraphs) {
            descriptionGraph.produceStartDLClauses(dlClauses);
        }
        return new DLOntology(ontologyURI, dlClauses, positiveFacts, negativeFacts,
                              determineExpressivity.m_hasInverseRoles,
                              determineExpressivity.m_hasAtMostRestrictions,
    ...
    protected static org.semanticweb.HermiT.model.Individual getIndividual(OWLIndividual individual) {
    ...
    protected static class Clausifier implements OWLDescriptionVisitor {
        ...
        public Clausifier(Set<Atom> positiveFacts,boolean renameAtMost,OWLDataFactory factory) {
        ...
        public void visit(OWLClass object) {
            m_headAtoms.add(Atom.create(AtomicConcept.create(object.getURI().toString()),new org.semanticweb.HermiT.model.Term[] { X }));
        }
        ...
        public void visit(OWLObjectAllRestriction object) {
            ...
            if (description instanceof OWLClass) {
                ...
                if (internal instanceof OWLClass) {
                    ...
                    m_bodyAtoms.add(Atom.create(AtomicConcept.create(owlClass.getURI().toString()),new org.semanticweb.HermiT.model.Term[] { y }));
                }
        ...
        public void visit(OWLObjectMaxCardinalityRestriction object) {
            if (m_renameAtMost) {
                ...
                } else throw new IllegalStateException("invalid normal form.");
                ...
                org.semanticweb.HermiT.model.Variable Y=nextY();
                m_bodyAtoms.add(getAbstractRoleAtom(object.getProperty(),X,Y));
                if (!AtomicConcept.THING.equals(toAtomicConcept)) m_bodyAtoms.add(Atom.create(toAtomicConcept,Y));
            } else addAtMostAtoms(object.getCardinality(),object.getProperty(),object.getFiller());
        }
        ...
        public void clausifyAtMostStuff(Collection<DLClause> dlClauses) {
            for (AtMostAbstractRoleGuard atMostAbstractRole : m_atMostAbstractRoleGuards) {
                ...
                addAtMostAtoms(atMostAbstractRole.getCaridnality(),onObjectProperty,m_factory.getOWLClass(URI.create(atMostAbstractRole.getToAtomicConcept().getURI().toString())));
        ...
        protected void addAtMostAtoms(int number,OWLObjectPropertyExpression onObjectProperty,OWLDescription toDescription) {
            ...
            if (toDescription instanceof OWLClass) {
                ...
                if (!(internal instanceof OWLClass)) throw new IllegalStateException("Invalid ontology normal form.");
                isPositive=false;
                atomicConcept=AtomicConcept.create(((OWLClass)internal).getURI().toString());
                if (AtomicConcept.NOTHING.equals(atomicConcept))
                    atomicConcept=null;
            }
            else throw new IllegalStateException("Invalid ontology normal form.");
    ...
    protected static class FactClausifier extends org.semanticweb.owl.util.OWLAxiomVisitorAdapter {
        protected final Set<Atom> m_positiveFacts;
    ...
    protected static class DetermineExpressivity implements OWLDescriptionVisitor {
        ...
        public void visit(OWLObjectComplementOf object) {
            // WARNING: I think that if this get executed our min/max tests don't work.
            // -rob 2008-06-16
            object.getOperand().accept(this);
        }
        public void visit(OWLObjectUnionOf object) {
            for (OWLDescription d : object.getOperands()) d.accept(this);
        }
        public void visit(OWLObjectIntersectionOf object) {
            for (OWLDescription d : object.getOperands()) d.accept(this);
        }
