...
public class NodesForCommand extends AbstractCommand implements ICommand {
    ...
    /**
     * Tries to fetch a concept name \"C\" from the arguments given to the 
     * constructor and prints a list of nodes and their blocking status for 
    ...
    public void execute() {
        ...
        while (node!=null) {
            ...
            if (existentialConcept instanceof AtLeastAbstractRoleConcept) {
                LiteralConcept toConcept = ((AtLeastAbstractRoleConcept)existentialConcept).getToConcept();
                if (toConcept.equals(atomicConcept)) {
                    if (index!=0) {
                        writer.print(",");
                        if (index % 5==0)
                            writer.println();
                    ...
                    Printing.printPadded(writer, node.getNodeID() + 
                            (node.isActive() ? "" : "*"), 8);
                    index++;
                }
            }
    ...
    public String getHelpText() {
        ...
        writer.println("Creates a concept \"C\" from conceptName and finds " +
        	       "all nodes that have been created by a concept " +
        	       "(atleast n r.C) and prints them and whether they are " +
        	       "active or not.");
        writer.flush();
