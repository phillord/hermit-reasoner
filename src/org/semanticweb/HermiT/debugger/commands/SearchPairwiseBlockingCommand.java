...
public class SearchPairwiseBlockingCommand extends AbstractCommand implements ICommand {
     ...
     * prints nodes and their blocking status that have the same pair-wise 
     * blocking signature of the node for the given node ID.
     */
    public void execute() {
        if (args.length < 2) {
        ...
        if (node!=null) {
            if (node.getNodeType()!=NodeType.TREE_NODE)
                debugger.getOutput().println("Node " + node.getNodeID() + 
                        " is not a tree node and does not have a pair-wise " +
                        "blocking signature.");
            else {
                ...
                while (searchNode!=null) {
                    if (searchNode.getNodeType() == NodeType.TREE_NODE
                            && node.getPositiveLabel().equals(
                                    searchNode.getPositiveLabel())
                            && node.getParent().getPositiveLabel().equals(
                                    searchNode.getParent().getPositiveLabel())
                            && node.getFromParentLabel().equals(
                                    searchNode.getFromParentLabel())
                            && node.getToParentLabel().equals(
                                    searchNode.getToParentLabel())
                            && node.getToSelfLabel().equals(
                                    searchNode.getToSelfLabel())) {
                        writer.print("  ");
    ...
    public String getHelpText() {
        ...
        writer.println("Prints nodes and their blocking status that have the " +
        	       "same pair-wise blocking signature of the node for " +
        	       "the given node ID. ");
        writer.flush();
