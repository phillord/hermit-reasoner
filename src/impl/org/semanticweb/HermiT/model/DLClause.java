...
/**
 * Represents a DL clause. The body is a conjunction of atoms and the head is a
...
public class DLClause implements Serializable {
    ...
    public DLClause getSafeVersion() {
        ...
        if (variables.isEmpty())
            ...
            Atom[] newBodyAtoms = new Atom[m_bodyAtoms.length
                    + variables.size()];
            System.arraycopy(m_bodyAtoms, 0, newBodyAtoms, 0,
                    m_bodyAtoms.length);
            int index = m_bodyAtoms.length;
            for (Variable variable : variables)
                newBodyAtoms[index++] = Atom.create(AtomicConcept.THING,
                        variable);
    ...
    public boolean isConceptInclusion() {
        if (getBodyLength() == 1 && getHeadLength() == 1) {
            if (getBodyAtom(0).getDLPredicate() instanceof AtomicConcept
                    && getHeadAtom(0).getDLPredicate() instanceof Concept) {
                ...
                if (getBodyAtom(1).getDLPredicate().equals(atomicAbstractRole)
                    ...
                    if (x != null && x.equals(getBodyAtom(1).getArgument(0))) {
                        ...
                        if (y1 != null
                                ...
                                && headY1 != null
                                && headY2 != null
                                && ((y1.equals(headY1) && y2.equals(headY2)) || (y1.equals(headY2) && y2.equals(headY1))))
                            return true;
                    }
                }
            }
    ...
    public boolean isRoleInclusion() {
        if (getBodyLength() == 1 && getHeadLength() == 1) {
            if (getBodyAtom(0).getDLPredicate() instanceof AtomicAbstractRole
                ...
                if (x != null && y != null && !x.equals(y) && x.equals(headX)
                        && y.equals(headY))
                    return true;
            }
        }
    ...
    public boolean isRoleInverseInclusion() {
        if (getBodyLength() == 1 && getHeadLength() == 1) {
            if (getBodyAtom(0).getDLPredicate() instanceof AtomicAbstractRole
                ...
                Variable headX = getHeadAtom(0).getArgumentVariable(0);
                Variable headY = getHeadAtom(0).getArgumentVariable(1);
                if (x != null && y != null && !x.equals(y) && x.equals(headY)
                        && y.equals(headX))
                    return true;
            }
        }
    ...
    protected static InterningManager<DLClause> s_interningManager = new InterningManager<DLClause>() {
        protected boolean equal(DLClause object1, DLClause object2) {
            if (object1.m_headAtoms.length != object2.m_headAtoms.length
                    || object1.m_bodyAtoms.length != object2.m_bodyAtoms.length)
                return false;
