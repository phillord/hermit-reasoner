...
public class AutomatonBuilderTest extends TestCase {
    ...
    public void testUpdateStateAfterCreationAndAddTransition() {
        ...
        assertTrue("automaton should accept word 'a'", a.accept(Collections.<Object> singletonList("a")));
    }
    ...
    public void testAddSeveralTransitions() {
        ...
        a.state("next").setTerminal(true);
        a.state("init").setInitial(true);
        assertTrue("automaton should accept word 'abcbc'",
                a.accept(Arrays.<Object> asList(new Object[] { "a", "b", "c", "b", "c" })));
        assertTrue("automaton should not accept word 'ab'",
                !a.accept(Arrays.<Object> asList(new Object[] { "a", "b" })));
    }
    ...
    public void testAddSeveralTransitionsFromSameStateAndLoops() {
        ...
        assertTrue("automaton should accept word 'accb'",
                a.accept(Arrays.<Object> asList(new Object[] { "a", "c", "c", "b" })));
        assertTrue("automaton should not accept word 'a'", !a.accept(Arrays.<Object> asList(new Object[] { "a" })));
    }

    public void testRestartBuilderFromOtherState() {
        Automaton a = new Automaton();
        a.setBuilder(new TransitionBuilder());
        a.from("init").on("a").go("next").on("b").go("other").from("next").on("c").loop().on("b").go("other");
        a.state("init").setInitial(true);
        a.state("other").setTerminal(true);
        assertTrue("automaton should accept word 'accb'",
                a.accept(Arrays.<Object> asList(new Object[] { "a", "c", "c", "b" })));
